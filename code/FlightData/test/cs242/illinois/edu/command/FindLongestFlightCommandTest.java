 ...
 * <p> Date: Sep 30, 2010
 *
 * <p> <p> Tests that the cs242.illinois.edu.command to find the longest flight works correctly
 */
public class FindLongestFlightCommandTest extends TestCase {
    ...
    /**
     * Tests that the longest flight we find is correct, based on the sample input data
     */
    public void testFindLongestFlightCommand() {

        //Setup
        FindLongestFlightCommand command = new FindLongestFlightCommand();
        ...
        for(int i = 0; i<castEdges.size(); i++){
            if(castEdges.get(i).getLength()>longestFlightLength){
                longestFlightLength = castEdges.get(i).getLength();
                longestFlight = castEdges.get(i);
            }
        }
        FlightGraphEdge expectedValue = longestFlight;

        //Test
        List<FlightGraphEdge> flights = FlightGraphUtilities.getFlightGraphEdges(graph);
        FlightGraphEdge actualValue = command.findLongestFlight(flights);
