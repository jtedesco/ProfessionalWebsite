 ...
 * <p> Date: Sep 30, 2010
 *
 * <p> <p> Tests the 'find smallest city' cs242.illinois.edu.command
 */
public class FindSmallestCityCommandTest extends TestCase {
    ...
    /**
     * Tests that the smallest city size we find is correct, based on the sample input data
     */
    public void testFindSmallestCitySizeCommand() {

        //Setup
        FindSmallestCityCommand command = new FindSmallestCityCommand();
        List<FlightGraphEdge> castEdges = new ArrayList();
        for(int i = 0; i<edges.size(); i++){
            castEdges.add((FlightGraphEdge) edges.get(i));
        }
        FlightGraphVertex expectedValue = (new FlightGraphVertex("SYD", "Sydney", "AU", "Asia", 10, new Pair('S', 34), new Pair('E', 151), 4475000, 4));

        //Test
        List<FlightGraphVertex> cities = FlightGraphUtilities.getFlightGraphVertices(graph);
        FlightGraphVertex actualValue = command.findSmallestCity(cities);
