 ...
 * <p> Date: Sep 30, 2010
 *
 * <p> <p> Test the 'find shortest flight' cs242.illinois.edu.command
 */
public class FindShortestFlightCommandTest extends TestCase {
    ...
    /**
     * Tests that the shortest flight we find is correct, based on the sample input data
     */
    public void testFindShortestFlightCommand() {

        //Setup
        FindShortestFlightCommand command = new FindShortestFlightCommand();
        ...
        for(int i = 0; i<castEdges.size(); i++){
            if(castEdges.get(i).getLength() < shortestFLightLength){
                shortestFLightLength = castEdges.get(i).getLength();
                shortestFlight = castEdges.get(i);
            }
        }
        FlightGraphEdge expectedValue = shortestFlight;

        //Test
        List<FlightGraphEdge> flights = FlightGraphUtilities.getFlightGraphEdges(graph);
        FlightGraphEdge actualValue = command.findShortestFlight(flights);
    ...
    private List<DirectedEdge> buildEdgeList() {
        ...
        //Add some sample airport data to a vertex list
        edgeList.add(new FlightGraphEdge(randomInt(), getRandomVertex(), getRandomVertex()));
        edgeList.add(new FlightGraphEdge(randomInt(), getRandomVertex(), getRandomVertex()));
        edgeList.add(new FlightGraphEdge(randomInt(), getRandomVertex(), getRandomVertex()));
        edgeList.add(new FlightGraphEdge(randomInt(), getRandomVertex(), getRandomVertex()));
        edgeList.add(new FlightGraphEdge(randomInt(), getRandomVertex(), getRandomVertex()));
