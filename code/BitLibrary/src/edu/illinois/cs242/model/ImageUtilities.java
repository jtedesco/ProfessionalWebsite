...
public class ImageUtilities {
    ...
    /**
     * Strips alpha channel from the input pixel data
     *
     * @param
     */
    public static int[][] stripAlphaChannel(int[][] inputData) {
        int[][] outputData = new int[inputData.length][inputData[0].length];

        for(int row = 0; row < inputData.length; row++){
            for(int col = 0; col < inputData[0].length; col++){
                outputData[row][col] = ((inputData[row][col] << ONE_BYTE) >>> ONE_BYTE);
            }
        }
	...
	public static BufferedImage loadImage(String filename) {
		try {
			...
			return ImageIO.read(ImageWindow.class.getResourceAsStream(filename));
		} catch (Exception e) {
		}
		try {
