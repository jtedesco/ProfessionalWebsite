     ...
     /**
       ...
       */
       public function renderResults($table_name, $column_names, $column_titles, $table_class, $highlighted_column_name,
          ...
          echo "<form name=\"hiddenform\" action=\"src/utils/sortable_table_redirect.php\" method=\"post\" style=\"display: none;\"><input name=\"sortcolumn\" type=\"text\" id=\"sortcolumn\" style=\"display: none;\"></form>";

          //Print out table tag with style class
          echo "<center><table class='$table_class'>";
          ...
          for($column_num = 0; $column_num < count($column_titles); $column_num++){
              ...
              $title = $column_titles[$column_num];
               ...
               // If the this column is the column currently sorted by
              if(strstr($sort_column, $name)==false){

                  echo "<td><a href=\"javascript:submit_hidden_form('$name ASC')\"/>$title</a></td>";

               } else {

                  if(strstr($sort_column, 'DESC')==false){

                       echo "<td><a href=\"javascript:submit_hidden_form('$name DESC')\"/>$title</a><span style='font-size: 14px; padding-bottom: 5px;'>&nbsp;&#9652</span></td>";
                   } else {

                       echo "<td><a href=\"javascript:submit_hidden_form('$name ASC')\"/>$title</a><span style='font-size: 14px; padding-bottom: 5px;'>&nbsp;&#9662</span></td>";
                   }
              }
          }
          echo "</thead>";
